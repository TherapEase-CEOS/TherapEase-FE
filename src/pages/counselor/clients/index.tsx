...
import { DUMMT_CLIENT_LIST, DUMMT_CLIENT } from '@/constants/DUMMY_DATA';

const ClientsPage = () => {
  // const { data: clientList, isLoading, isFetching, isError } = useClientList();

  const [clientList, setClientList] = useState<IClient[]>(DUMMT_CLIENT_LIST);
  ...
  const [selectedClient, setSelectedClient] = useState<IClient>(DUMMT_CLIENT);
  ...
  const handleAddSubmit = () => {
    ...
    const newClient: IClient = {
      clientId: Math.random().toString(36).substring(2, 10), // 임시 ID
      name: addInputValue.trim(),
      status: 'ongoing',
  ...
  const onDeleteClient = (id: string) => {
    ...
    setClientList((prev) => prev.filter((client) => client.clientId !== id));
    setIsDeleteModalVisible(false);
    showAlert('내담자가 삭제되었습니다.');

    setIsDeleteModalVisible(false);
  };

  // 내담자 완료
  const handleCompleteClient = (id: string) => {
    // TODO - 내담자 완료 api 연동
    // TODO - 내담자 리스트 get api 연동

    setClientList((prev) =>
      prev.map((client) =>
        client.clientId === id ? { ...client, status: 'completed' } : client,
      ),
    );
    showAlert('내담자가 완료 처리되었습니다.');
  };
  const filteredClients = clientList?.filter((client: IClient) =>
  ...
  "
    >
      ...
      {isLoading || isFetching ? (
        ...
        <div className="text-center mt-[22.1rem] text-heading3 text-gray-5">
          <span>내담자 정보를 불러오지 못했습니다.</span>
        </div>
      ) : clientList.length === 0 ? (
        ...
        <div className="w-full flex flex-wrap justify-center gap-[1.6rem] mt-[3.236rem]">
          {filteredClients.map((client: IClient) => (
            <div className="card" key={client.clientId}>
              <ClientCard
                setCompleteClient={handleCompleteClient}
                clientInfo={client}
                detailMenu={true}
